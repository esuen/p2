...
func NewShadowTrafficHealthChecker(hClient HealthClient, resourceClient ResourceClient, cClient consulutil.ConsulClient, rcStore ReplicationControllerStore, labelReader LabelReader) ShadowTrafficHealthChecker {
...
func (h shadowTrafficHealthChecker) WatchService(
	...
	if useHealthService {
		...
		statusEndpoints, err := h.getStatusEndpoints(serviceID, status, logger)
		if err != nil {
			select {
			case <-ctx.Done():
				return
		...
		logger.NoFields().Debugf("WatchService-useHealthService: statusEndpoints: %s", statusEndpoints)
		...
		go func() {
			for {
				...
				case <-ctx.Done():
					return
				case resp := <-respChan:
					logger.NoFields().Debugf("WatchService-useHealthService: Received health check from health service for %s with health %s and response %v", resp.HealthRequest.Url, resp.Health, resp)
					out := make(map[types.NodeName]health.Result)
		...
		go func() {
			...
			for {
				logger.NoFields().Debugf("WatchService-useHealthService: creating health monitors for endpoints: %s", statusEndpoints)
				err := h.healthClient.HealthMonitorEndpoints(ctx, healthReq, respChan)
				logger.Debugf("WatchService-useHealthService: got an error with HealthMonitorEndpoints call: %v", err)
				if err != nil {
					select {
					case <-ctx.Done():
						return
...
func (h shadowTrafficHealthChecker) Service(
	...
	// health service
	// get status endpoints for service
	statusEndpoints, err := h.getStatusEndpoints(serviceID, status, logging.NewLogger(logrus.Fields{}))
	if err != nil {
		return nil, err
	}
	protocol := "HTTPS"
